...
import { GET_INSIGHT_BY_PATH } from '@/lib/queries'
import { DrupalInsight } from '@/lib/types'
...
interface InsightByPathData {
  route: {
    entity: DrupalInsight
  } | null
}

async function getInsight(path: string): Promise<DrupalInsight | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<InsightByPathData>({
      query: GET_INSIGHT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getInsight(path)

  if (!item) {
    return { title: 'Insight Not Found | Consulting' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).insightCategory && (
                    <div>
                      <dt className="text-sm text-gray-500">Insight Category</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).insightCategory}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
