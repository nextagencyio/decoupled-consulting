...
import { GET_CASE_STUDY_BY_PATH } from '@/lib/queries'
...
interface CaseStudyByPathData {
  route: {
    entity: DrupalCaseStudy
  } | null
}
...
async function getCaseStudy(path: string): Promise<DrupalCaseStudy | null> {
  try {
    ...
    const { data } = await apolloClient.query<CaseStudyByPathData>({
      query: GET_CASE_STUDY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/case-studies/${slug.join('/')}`
  const item = await getCaseStudy(path)

  if (!item) {
    return { title: 'Case Study Not Found | Consulting' }
  }

  return {
    title: `${item.title} | Consulting`,
                ...
                <dl className="space-y-4">
                  {(item as any).clientName && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).clientName}</dd>
                    </div>
                  )}
                  {(item as any).industry && (
                    <div>
                      <dt className="text-sm text-gray-500">Industry</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).industry}</dd>
                    </div>
                  )}
                  {(item as any).resultsSummary && (
                    <div>
                      <dt className="text-sm text-gray-500">Results Summary</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).resultsSummary}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
